...
const VERSION = "phase3-devtools-2025-12-22";

const app = express();
app.get("/", (_req, res) => res.status(200).send("barikade-server ok"));
app.get("/health", (_req, res) => {
  let roomsCount = rooms.size;
  let clients = 0;
  for(const r of rooms.values()) clients += r.clients.size;
  res.status(200).json({ ok: true, ts: Date.now(), version: VERSION, rooms: roomsCount, clients });
});
...
function sendError(ws, code, message){
  try{ ws.send(JSON.stringify({type:'error', code, message})); }catch(_e){}
}
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (buf) => {
    ...
    if(type === "join"){
      ...
      if(!roomCode) return sendError(ws, "ROOM_MISSING", "Room fehlt");
    ...
    const room = ws._room ? getRoom(ws._room) : null;
    if(!room) return sendError(ws, "NOT_IN_ROOM", "Nicht in einem Raum");

    if(type === "start"){
      if(!isHost(room, clientId)) return sendError(ws, "ONLY_HOST", "Nur Host kann starten");
      const activePlayers = room.players.filter(p => p.color);
      if(activePlayers.length < 2) return sendError(ws, "NEED_2_PLAYERS", "Mindestens 2 Spieler nötig");
      room.started = true;
      room.paused = false;
      room.lastRoll = null;
      room.awaitingBarricade = false;
